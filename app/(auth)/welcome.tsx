...
const Welcome = () => {
  ...
  const buttonHandler = () => (isLastSlide ? router.replace("/(auth)/sign-up") : swiperRef.current?.scrollBy(1));
  ...
  return (
    <SafeAreaView className="flex h-full items-center justify-between bg-white">
      ...
      <View className=" w-full px-10">
        <CustomButton onPress={buttonHandler} title={isLastSlide ? "Get Started" : "Next"} className="w-11/12 mt-10" />
      </View>
    </SafeAreaView>
  );
};
